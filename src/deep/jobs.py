...
class DeepJob:
    ...
    def load(cls, root: Path) -> DeepJob:
        ...
        return cls(root, raw["source"], raw["baseline"], Path(raw["installation"]), DeepSettings(**raw["settings"]), raw["phase"])

    def save(self) -> None:
        self.root.mkdir(parents=True, exist_ok=True)
        raw = {"source": self.source, "baseline": self.baseline, "installation": str(self.installation), "settings": self.settings.to_dict(), "phase": self.phase}
