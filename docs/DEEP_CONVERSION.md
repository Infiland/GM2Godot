...
Closing GM2Godot requests a graceful pause. Reopening offers saved jobs; **Deep → Resume saved job** also opens them. Resume can raise cumulative token/cost limits, grant more running time and adjust concurrency. Provider/model changes require a new job. Changed source or baseline hashes currently require a new job; selective cross-job reuse is not implemented.
