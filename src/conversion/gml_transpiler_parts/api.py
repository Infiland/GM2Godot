...
def transpile_gml_code_with_source_map(
    ...
    static_declarations = (
        collect_static_declarations(tokens) if static_scope_prefix is not None else ()
    )
    ...
    if static_declarations:
        ...
        static_scope_id = build_static_scope_id(static_scope_prefix, None, 0, tokens)
        static_scope_name = (
    ...
    statement_result = parse_gml_statements(
        GMLStatementRequest(
            tokens=tuple(tokens),
            local_names=frozenset(initial_local_names),
            instance_variables=instance_variables,
            inherited_event_call=inherited_event_call,
            return_depth=return_depth,
            macro_configuration=macro_configuration,
            top_level_global_scope=top_level_global_scope,
            global_names=(
                frozenset(LEGACY_GLOBAL_BUILTINS)
                if legacy_global_builtins
                else frozenset()
            ),
            enum_values=known_enum_values,
            enum_names=frozenset(known_enum_values),
            ...
            asset_names=frozenset(asset_names or ()),
            ...
            extension_function_mappings=normalize_extension_function_mappings(
                extension_function_mappings
            ),
        )
    )
    lines = list(statement_result.lines)
    if static_declarations:
        lines = [
            *emit_static_initialization_lines(
                ...
                static_declarations,
                initial_local_names,
                statement_result.scope_context,
                statement_result.enum_values,
                statement_result.enum_names,
                statement_result.macro_values,
            ),
            *lines,
        ]
    ...
    if not lines:
        ...
        code = "\n".join(prefix_multiline(line, indent) if line else "" for line in lines)
