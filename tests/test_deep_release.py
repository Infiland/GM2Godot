...
class DeepReleaseTests(unittest.TestCase):
    def _bundles(self, root: Path, test_build: bool = False) -> None:
        for platform in PLATFORMS:
            name = f"gm2godot-deep-0.2.0-{platform}.zip"
            ...
            (root / f"manifest-{platform}.json").write_text(json.dumps({
                ...
                "packages": {platform: {
                    "url": f"https://github.com/Infiland/GM2Godot/releases/download/deep-v0.2.0/{name}",
    ...
    def test_seals_exact_platform_set_and_rejects_changed_bytes(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            self._bundles(root)
            path = seal_manifest(root, "a" * 40, "b" * 40, "0.2.0")
            self.assertEqual(set(json.loads(path.read_text())["packages"]), set(PLATFORMS))
            (root / "gm2godot-deep-0.2.0-win-x64.zip").write_bytes(b"modified")
            with self.assertRaisesRegex(ValueError, "checksum"):
                seal_manifest(root, "a" * 40, "b" * 40, "0.2.0")
