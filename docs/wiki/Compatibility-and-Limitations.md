...
The separate destination-wide `.gm2godot-managed-output.lock` and `.gm2godot-managed-output/` workspace added in 0.7.38 remain internal: production converters do not use that stage yet. Version 0.7.39 added the immutable complete managed-generation inventory consumed by selective carry-forward, staged/public validation, manifest rendering, and publication guards. Version 0.7.40 adds the publisher that durably journals verified same-filesystem stages, backups, exact prior/desired inventory records, and attempt/manifest receipts before any public move. It installs the synthetic managed generation and canonical evidence with the manifest last, then uses one persistent pointer as the old-or-new decision; rollback or restart before that pointer restores the prior generation, while recovery after it verifies the new generation and performs identity-bound cleanup. Rollback ambiguity preserves exact material and emits a distinct bounded recovery artifact. Converter orchestration, exhaustive process-kill/native-platform integration, and final operator behavior remain future transaction work; failed or cancelled production conversions retain their pre-integration destination-wide behavior outside the already-transactional artifact pairs described above.
