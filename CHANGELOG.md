...
- Replaced cross-phase raw expression-emission tuples with the frozen `GMLExpressionEmission` result, retained the canonical #816 AST union through its `GMLExpression` alias, and migrated all higher-level expression consumers without adding a top-level facade export.
- Advanced the architecture baseline from 96 to 60 private edges across 14 module pairs while keeping all 60 production imports, reducing private production import edges from 7 to 4 and tracked private-usage suppressions from 12 to 8, with generated bytes, diagnostics, locations, source maps, static IDs, golden output, and all 74 facade exports/signatures unchanged.
