...
> **Applies to:** GM2Godot 0.7.70 · GameMaker LTS 2026 · Godot 4.7.2
>
...
These rules follow GameMaker LTS [Method Variables](https://manual.gamemaker.io/lts/en/GameMaker_Language/GML_Overview/Method_Variables.htm), [Instance Keywords](https://manual.gamemaker.io/lts/en/GameMaker_Language/GML_Overview/Instance_Keywords.htm), and [`method`](https://manual.gamemaker.io/lts/en/GameMaker_Language/GML_Reference/Variable_Functions/method.htm). Godot behavior is pinned to the exact 4.7.2 [`Callable`](https://docs.godotengine.org/en/4.7/classes/class_callable.html) contract—where lambdas, bound arguments, global functions, and Variant methods can all be custom callables—and its [GDScript lambda](https://docs.godotengine.org/en/4.7/tutorials/scripting/gdscript/gdscript_basics.html#lambda-functions) rules.
...
Version 0.7.58 extends those checks to the backup lifecycle. A staged backup is revalidated after the observable `after_backup` hook, every backup is revalidated as one complete set before the first public mutation, and the applicable backup is checked again before its ordered commit and after `before_commit`. An ordinary error or control signal from backup staging durably removes only the exact owned stage; an in-place change, identity replacement, hard link, or other unverified same-name path is preserved and named in the failure instead of being trusted or deleted. Publication, snapshot restoration, and stale conversion-artifact retirement cannot return success when exact cleanup or its required durability barrier is incomplete, even if the public files already committed successfully.

Cancellation remains first-class across this transaction. Publication, restoration, reverse rollback and cleanup preserve the exact first `KeyboardInterrupt`, `SystemExit`, or other control-flow exception, including object identity and `SystemExit.code`; rollback still attempts every recorded mutation, while later failures and retained recovery paths are attached as notes. If a replace or unlink completed before the signal became observable, the transaction records the resulting ownership before rollback, and completed cleanup unlinks reach their applicable directory durability barrier before the exact signal is re-raised. Repeated exact probes resolve interrupted completion where possible; genuinely indeterminate private-stage ownership is preserved rather than guessed. An earlier ordinary operation error remains primary over a later ordinary cleanup error, with the cleanup details attached, while internal recovery-stage cleanup keeps its deliberate retry behavior.
...
| Shaders and GPU state | Supported 2D attributes, varyings, uniforms, base-texture sampling, and matrix transforms compile under exact Godot 4.7.2; custom vertex streams, macros, 3D/clip-space logic, multi-pass effects, GPU state, and visual parity remain manual review areas. |
