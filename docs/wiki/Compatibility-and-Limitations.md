...
The separate destination-wide `.gm2godot-managed-output.lock` and `.gm2godot-managed-output/` workspace added in 0.7.38 remain an internal foundation: production converters do not use that stage yet. Version 0.7.39 adds the immutable complete managed-generation inventory consumed by selective carry-forward, staged/public validation, manifest rendering, and manifest publication guards. The inventory is deterministic across enumeration order, worker count, host separators, and unchanged reruns; it records exact bytes, hashes, modes, and converter/shared ownership while excluding private transaction state and unrelated files. Destination-wide recoverable publication, converter orchestration through the stage, and crash-boundary integration remain future transaction work; failed or cancelled conversions retain their pre-transaction destination-wide behavior outside the already-transactional artifact pairs described above.
