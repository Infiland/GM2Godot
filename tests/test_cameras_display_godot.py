...
def _write_multiview_scene(project_dir: Path) -> None:
    smoke_script = textwrap.dedent(
        """\
        extends Node2D
        ...
        func _has_diagnostic(needle):
        \tfor diagnostic in GMRuntime.gml_view_backend_diagnostics():
        \t\tif str(diagnostic).find(str(needle)) >= 0:
        \t\t\treturn true
        \treturn false

        func _ready():
        \tGMRuntime.gml_draw_begin(self, "_draw")
        \tvar cam0 = GMRuntime.gml_view_get_camera(0)
        \tvar cam1 = GMRuntime.gml_view_get_camera(1)
        \tif not _check(GMRuntime.gml_handle_is_valid(cam0) and GMRuntime.gml_handle_is_valid(cam1), "converted view cameras were not resolved"):
        \t\treturn
        \tif not _check(GMRuntime.gml_view_get_visible(0) and GMRuntime.gml_view_get_visible(1), "view visibility metadata did not initialize"):
        \t\treturn
        \tif not _check(GMRuntime.gml_view_get_xport(1) == 400 and GMRuntime.gml_view_get_yport(1) == 20, "view port origin metadata mismatch"):
        \t\treturn
        \tif not _check(GMRuntime.gml_view_get_wport(1) == 640 and GMRuntime.gml_view_get_hport(1) == 360, "view port size metadata mismatch"):
        \t\treturn
        \tGMRuntime.gml_view_set_visible(0, true)
        \tGMRuntime.gml_view_set_visible(1, true)
        \tif not _check(_has_diagnostic("multiple active GameMaker views"), "multi-view backend diagnostic was not recorded"):
        \t\treturn
        \tGMRuntime.gml_view_set_xport(1, 400)
        \tGMRuntime.gml_view_set_yport(1, 20)
        \tGMRuntime.gml_view_set_wport(1, 640)
        \tGMRuntime.gml_view_set_hport(1, 360)
        \tGMRuntime.gml_window_mouse_set(410, 120)
        \tif not _check(GMRuntime.gml_window_mouse_get_x() == 410 and GMRuntime.gml_window_mouse_get_y() == 120, "window mouse state mismatch"):
        \t\treturn
        \tif not _check(GMRuntime.gml_window_views_mouse_get_x() == 410 and GMRuntime.gml_window_views_mouse_get_y() == 120, "active-views mouse state mismatch"):
        \t\treturn
        \tGMRuntime.gml_display_set_gui_size(800, 450)
        \tif not _check(GMRuntime.gml_device_mouse_x_to_gui(0) > 0 and GMRuntime.gml_device_mouse_y_to_gui(0) > 0, "GUI mouse conversion failed under multi-view state"):
        \t\treturn
        \tif not _check(abs(GMRuntime.gml_window_view_mouse_get_x(1) - 325.0) < 0.01, "view mouse x conversion mismatch"):
        \t\treturn
        \tif not _check(abs(GMRuntime.gml_window_view_mouse_get_y(1) - 50.0) < 0.01, "view mouse y conversion mismatch"):
        \t\treturn
        \tvar surf = GMRuntime.gml_surface_create(64, 64)
        \tif not _check(GMRuntime.gml_view_set_surface_id(1, surf) == surf, "view surface assignment failed"):
        \t\treturn
        \tif not _check(GMRuntime.gml_view_get_surface_id(1) == surf, "view surface readback mismatch"):
        \t\treturn
        \tif not _check(_has_diagnostic("view_surface_id"), "view surface backend diagnostic was not recorded"):
        \t\treturn
        \tGMRuntime.gml_view_set_surface_id(1, -1)
        \tGMRuntime.gml_surface_free(surf)
        \tGMRuntime.gml_draw_end()
        \tprint("MULTIVIEW_DISPLAY_SMOKE_OK")
        \tget_tree().quit(0)
        """
    )
    smoke_scene = textwrap.dedent(
        ...
        metadata/gamemaker_view_wport = 320
        metadata/gamemaker_view_hport = 180

        [node name="ViewCamera_2" type="Camera2D" parent="."]
        position = Vector2(480, 90)
        enabled = false
        metadata/gamemaker_view_camera = true
        metadata/gamemaker_view_enabled_camera = false
        metadata/gamemaker_view_visible = true
        metadata/gamemaker_view_index = 1
        metadata/gamemaker_view_xview = 320
        metadata/gamemaker_view_yview = 0
        metadata/gamemaker_view_wview = 320
        metadata/gamemaker_view_hview = 180
        metadata/gamemaker_view_xport = 400
        metadata/gamemaker_view_yport = 20
        metadata/gamemaker_view_wport = 640
        metadata/gamemaker_view_hport = 360
        """
    )
    _write_text(project_dir / "smoke.gd", smoke_script)
    _write_text(project_dir / "smoke.tscn", smoke_scene)
...
class TestCamerasDisplayGodotSmoke(unittest.TestCase):
    ...
    def test_multiview_viewport_state_and_diagnostics(self) -> None:
        returncode, output = self._run_smoke(_write_multiview_scene)
        self.assertEqual(returncode, 0, output)
        self.assertIn("MULTIVIEW_DISPLAY_SMOKE_OK", output)
