...
This publisher remains an internal surface in 0.7.40. Production converters, finalizers, `Converter.convert()`, and the CLI still write through their existing path until the next integration step; exhaustive process-kill/native-platform orchestration and operator documentation follow later. The publisher also does not define logical stale-resource invalidation. Versions 0.7.39 and 0.7.40 therefore change generation evidence and transaction infrastructure, not GameMaker conversion or generated Godot runtime semantics.
