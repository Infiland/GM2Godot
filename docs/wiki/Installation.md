...
After launch, confirm that the title bar or **Help → About GM2Godot** shows version `0.7.40`.
...
The first command should print `GM2Godot 0.7.40`; the second should list the conversion groups and the exact converter keys accepted by `--only`. The same CLI is also available through `python -m src.cli`.
